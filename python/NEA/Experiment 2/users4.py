...
if attempt==0:                               # unsuccessful entry of user names
    ...
    admin=input("Would you like to register with the administrator of the game? ")
    if admin == "y".upper():
        import subprocess
        subprocess.call(["python", "usersAdmin4.py"])
        gameReturn = input("""Now do you want to play?
Y or N: """)
        if gameReturn == "y".upper():
            pass
        elif gameReturn == "n".upper():
            return
else:
